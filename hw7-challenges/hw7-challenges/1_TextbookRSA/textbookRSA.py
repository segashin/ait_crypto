...
class TextbookRSA:
    ...
    def encrypt(self, msg):
        #msgNum = int(msg.encode('hex'), base=16)
        msgNum = int(msg.hex(), base=16)
    ...
    def decrypt(self, num):
        ...
        msg = bytes.fromhex(hex(msgNum)[2:])
        return msg
...
if __name__ == "__main__":      
    ...
    rsa = TextbookRSA()
    op_end = time.time()  
    print("\nKey pair generation took " + str(op_end-op_start) + " seconds.")
    ...
    op_end = time.time()
    print("\nCiphertext computed (in " + str(op_end-op_start)+ " seconds): ")
    print(ciphertext)

    op_start = time.time()
    plaintext_recovered = rsa.decrypt(ciphertext).decode('utf-8')
    op_end = time.time()
    print("\nPlaintext recovered (in " + str(op_end-op_start) + " seconds): " + plaintext_recovered + "\n")
